import { Input, Query } from '@angular/core';
...
export class HomeComponent implements OnInit {
  ...
  highest() {
    console.log("HI");
    this.movies = this.movies.sort(function (m, n) {
      return n.rating - m.rating
    })
  }
  ...
  oldDate() {
    this.movies = this.movies.sort(function (m, n) {
      return m.release_date - n.release_date
    })
  }

  newDate() {
    this.movies = this.movies.sort(function (m, n) {
      return n.release_date - m.release_date
    })
  }
  ...
  searchClick(e: KeyboardEvent): void {
    ...
    if (this.query.length > 0) {
      this.movies = this.movies.filter((m) => {
        return m.title.includes(this.query)
      })
    }
    else {
      this.movies = this.moviecopy
    }
