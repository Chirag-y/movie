...
export class MoviescardComponent implements OnInit {
  ...
  @Output() inde: EventEmitter<Movie> = new EventEmitter();
  ...
  onClick(movie: Movie) {
    this.indexGet.emit(movie)
    this.inde.emit(movie)
    console.log(this.movie.id);
    localStorage.setItem("MovieID", this.movie.id)
    localStorage.setItem("MovieGenre", JSON.stringify(this.movie.genres))
    localStorage.setItem("Genre", this.movie.genres)
    this.id = this.route.snapshot.paramMap.get(this.movie.title)
    localStorage.setItem("Cast", JSON.stringify(this.movie.cast))
    this.movieCast = localStorage.getItem("Cast")
